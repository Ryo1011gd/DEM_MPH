...
//    char buf[1024];
    ...
    Cuboid2_t cub2;
    int spaceflag=0;
    int typeflag=0, lowerflag=0, upperflag=0, veloflag=0, enthalpyflag=0, angleofflag=0;
    cub2.type = iType_t(0);
    ...
    while(1){
        ...
        if(strcmp(token,endcommand)==0)break;
            ...
            veloflag=1;
             ...
             if(fscanf(fp,"%lf",&cub2.angle)!=1)goto err;
            angleofflag=1;
        }
      //  else if(strcmp(token,"Enthalpy")==0){
    ...
    if(upperflag==0)fprintf(stderr,"no indecatio to Upper");
    if(veloflag==0)fprintf(stderr,"no indecatio to Velocity");
    if(angleofflag==0)fprintf(stderr,"no indecatio to Angle");
   // if(enthalpyflag==0)fprintf(stderr,"no indecatio to Enthalpy");


   
    if(!(spaceflag && typeflag && lowerflag && upperflag && veloflag && angleofflag   ))return 1;
        fprintf(stderr, "line %d\n", __LINE__);
    Cuboid2 += cub2;
    return 0;
...
for (iCub2_t iCub2(0); iCub2 < cubcount2; ++iCub2) {
    ...
    const vec3<int> count = vec3<int>(
        round(width[0] / cub2.space),
        round(width[1] / cub2.space),
        round(width[2] / cub2.space)
    );
    const vec_t spacing = vec_t(
        width[0] / count[0],
        width[1] / count[1],
        width[2] / count[2]
    );
    ...
    vec_t center = (cub2.upper + cub2.lower) / 2;

    // Adjusting the loop to generate positions with rotation around the center
    for (double px = cub2.lower[0] + 0.01 * spacing[0]; px < cub2.upper[0] - 0.0 * spacing[0]; px += spacing[0]) {
        for (double py = cub2.lower[1] + 0.01 * spacing[1]; py < cub2.upper[1] - 0.0 * spacing[1]; py += spacing[1]) {
            for (double pz = cub2.lower[2] + 0.5 * spacing[2]; pz < cub2.upper[2] - 0.49 * spacing[2]; pz += spacing[2]) {
                ...
                // Calculate rotated positions around the center
                double new_px = (px - center[0]) * cos(cub2.angle) - (py - center[1]) * sin(cub2.angle) + center[0];
                double new_py = (px - center[0]) * sin(cub2.angle) + (py - center[1]) * cos(cub2.angle) + center[1];

            //    fprintf(stderr, "p %e %e %e\n", new_px, new_py, pz);
                Type += cub2.type;
                Position += vec_t(new_px, new_py, pz);
...
for (iCyb_t iCyb(0); iCyb < cybcount; ++iCyb) {
    ...
    double side_length = fmin(width[0], fmin(width[1], width[2])) / sqrt(3.0);


    // Calculate the number of triangles that can fit on each face
    int num_triangles_x = static_cast<int>(floor(width[0] / side_length));
    int num_triangles_y = static_cast<int>(floor(width[1] / side_length));
    int num_triangles_z = static_cast<int>(floor(width[2] / side_length));

    // Calculate the spacing between triangles
    vec_t spacing = vec_t(width[0] / num_triangles_x, width[1] / num_triangles_y, width[2] / num_triangles_z);

    for (int i = 0; i < num_triangles_x; ++i) {
        for (int j = 0; j < num_triangles_y; ++j) {
            for (int k = 0; k < num_triangles_z; ++k) {
                // Calculate the position of the triangle's centroid
                double px = cyb.lower[0] + (i + 0.5) * spacing[0];
                double py = cyb.lower[1] + (j + 0.5) * spacing[1];
                double pz = cyb.lower[2] + (k + 0.5) * spacing[2];

                // Output the vertices of the equilateral triangle
             //   fprintf(stderr, "v %e %e %e\n", px, py - side_length / sqrt(3.0), pz);  // Top vertex
            //    fprintf(stderr, "v %e %e %e\n", px - side_length / 2.0, py + side_length / (2.0 * sqrt(3.0)), pz);  // Bottom-left vertex
            //    fprintf(stderr, "v %e %e %e\n", px + side_length / 2.0, py + side_length / (2.0 * sqrt(3.0)), pz);  // Bottom-right vertex
                // Update any necessary variables (e.g., Type, Position, Velocity, etc.)
                Type += cyb.type;
...
for (iCyb2_t iCyb2(0); iCyb2 < cybcount2; ++iCyb2) {
    ...
    const double spacing = cyb2.space;
    const double height = width[2];

    // 上面（upper）平面から外側円半径 R1 を求める（例として x,y 平面半径の平均）
    const double R1 = 0.5 * (width[0] + width[1]) * 0.5;  // 半径 = 平均直径 / 2
    const double R2 = R1 * cyb2.ratio;  // 下面の半径は R1 * ratio

    // 側面（円錐側面）配置
    for (double pz = cyb2.lower[2] + 0.5 * spacing; pz < cyb2.upper[2] - 0.49 * spacing; pz += spacing) {
        double ratio_z = (pz - cyb2.lower[2]) / height;
        double radius = R2 + (R1 - R2) * ratio_z;  // 高さに応じて半径線形補間

        double circumference = 2.0 * M_PI * radius;
        int n_theta = std::max(6, static_cast<int>(circumference / spacing));

        for (int ith = 0; ith < n_theta; ith++) {
            double theta = (2.0 * M_PI * ith) / n_theta;
            double px = center[0] + radius * cos(theta);
            double py = center[1] + radius * sin(theta);
